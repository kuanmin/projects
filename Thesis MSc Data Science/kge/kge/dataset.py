...
class Dataset(Configurable):
    ...
    def load_triples(self, key: str) -> Tensor:
        ...
        if key not in self._triples:
            ...
            triples_pri = Dataset._load_triples(
                os.path.join(self.folder, filename),
                use_pickle=self.config.get("dataset.pickle"),
            )
            if key == "train":
                relation = self.config.get("dataset_limited.train.relation")
                if relation > 0:
                    condition = triples_pri[:, 1] == relation
                    triples = triples_pri[condition, :]
                    self.config.log(f"Loaded {len(triples)} {key} triples")
                    self._triples[key] = triples
                else:
                    self.config.log(f"Loaded {len(triples_pri)} {key} triples")
                    self._triples[key] = triples_pri

            if key == "valid":
                relation = self.config.get("dataset_limited.valid.relation")
                if relation > 0:
                    condition = triples_pri[:, 1] == relation
                    triples = triples_pri[condition, :]
                    self.config.log(f"Loaded {len(triples)} {key} triples")
            ...
            if key == "test":
                relation = self.config.get("dataset_limited.test.relation")
                if relation > 0:
                    condition = triples_pri[:, 1] == relation
                    triples = triples_pri[condition, :]
                    self.config.log(f"Loaded {len(triples)} {key} triples")
