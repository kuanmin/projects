...
class TrainingJob(Job):
    ...
    def __init__(
        ...
        if self.config.get("model") in [
            "averageensemble",
            "logisticregressionensemble",
        ]:
            ### build optimizer for meta and base learners
            if self.config.get("model") == "logisticregressionensemble":
                import torch.optim

                optimizer = getattr(
                    torch.optim,
                    config.get("logisticregressionensemble.meta_optimizer"),
                )
                self.optimizer_meta = optimizer(
                    [self.model.aggregate_coef],
                    **config.get("logisticregressionensemble.meta_optimizer_args"),
                )
                if self.config.get("logisticregressionensemble.updata_lr_scalar"):
                    lr_temp = self.model.config_01.get("train.optimizer_args.lr")
                    self.model.config_01.set(
                        "train.optimizer_args.lr",
                        lr_temp
                        * self.config.get("logisticregressionensemble.lr_scalar"),
                    )
                    lr_temp = self.model.config_02.get("train.optimizer_args.lr")
                    self.model.config_02.set(
                        "train.optimizer_args.lr",
                        lr_temp
                        * self.config.get("logisticregressionensemble.lr_scalar"),
                    )
                    if self.model.num_base > 2:
                        lr_temp = self.model.config_03.get("train.optimizer_args.lr")
                        self.model.config_03.set(
                            "train.optimizer_args.lr",
                            lr_temp
                            * self.config.get("logisticregressionensemble.lr_scalar"),
                        )
                    if self.model.num_base > 3:
                        lr_temp = self.model.config_03.get("train.optimizer_args.lr")
                        self.model.config_03.set(
                            "train.optimizer_args.lr",
                            lr_temp
            ...
            self.optimizer_base_01 = KgeOptimizer.create(
                self.model.config_01, self.model.model_base_01
            )
            self.optimizer_base_02 = KgeOptimizer.create(
                self.model.config_02, self.model.model_base_02
            )
            if self.model.num_base > 2:
                self.optimizer_base_03 = KgeOptimizer.create(
                    self.model.config_03, self.model.model_base_03
                )
            if self.model.num_base > 3:
                self.optimizer_base_04 = KgeOptimizer.create(
                    self.model.config_04, self.model.model_base_04
                )
        ...
        if self.config.get("model") in [
            ...
            ###build kge_lr_scheduler for meta and base learners
            if self.config.get("model") == "logisticregressionensemble":
                self.kge_lr_scheduler_meta = KgeLRScheduler(config, self.optimizer_meta)
            self.kge_lr_scheduler_base_01 = KgeLRScheduler(
                self.model.config_01, self.optimizer_base_01
            )
            self.kge_lr_scheduler_base_02 = KgeLRScheduler(
                self.model.config_02, self.optimizer_base_02
            )
            if self.model.num_base > 2:
                self.kge_lr_scheduler_base_03 = KgeLRScheduler(
                    self.model.config_03, self.optimizer_base_03
                )
            if self.model.num_base > 3:
                self.kge_lr_scheduler_base_04 = KgeLRScheduler(
                    self.model.config_04, self.optimizer_base_04
                )
    ...
    def create(
        ...
        if (
            config.get("model") == "logisticregressionensemble"
            and config.get("freeze_param") == "base"
        ):
            return TrainingJobMetaLearner(config, dataset, parent_job, model=model)
        else:
            if config.get("train.type") == "KvsAll":
                ...
                return TrainingJobNegativeSampling(
                    config, dataset, parent_job, model=model
                )
    ...
    def _run(self) -> None:
        ...
        if self.config.get("model") in [
            ...
            if self.config.get("model") == "logisticregressionensemble":
                # when "logisticregressionensemble.updata_lr" is set to TRUE, update the learning rate of the meta learner
                # this design is helpful while resuming to a pretrained ensemble.
                if self.model.config.get("logisticregressionensemble.updata_lr"):
                    self.optimizer_meta.param_groups[0]["lr"] = self.model.config.get(
                        "logisticregressionensemble.meta_optimizer_args.lr"
                    )
                # when "logisticregressionensemble.updata_lr_scalar" is set to TRUE, update the learning rates of the base learner all together with a scalar
                if self.model.config.get("logisticregressionensemble.updata_lr_scalar"):
                    lr_ori = self.optimizer_base_01.param_groups[0]["lr"]
                    self.optimizer_base_01.lr = lr_ori * self.config.get(
                        "logisticregressionensemble.lr_scalar"
                    )
                    lr_ori = self.optimizer_base_02.param_groups[0]["lr"]
                    self.optimizer_base_02.lr = lr_ori * self.config.get(
                        "logisticregressionensemble.lr_scalar"
                    )
                    if self.model.num_base > 2:
                        lr_ori = self.optimizer_base_03.param_groups[0]["lr"]
                        self.optimizer_base_03.lr = lr_ori * self.config.get(
                            "logisticregressionensemble.lr_scalar"
                        )
                    if self.model.num_base > 3:
                        lr_ori = self.optimizer_base_04.param_groups[0]["lr"]
                        self.optimizer_base_04.lr = lr_ori * self.config.get(
                            "logisticregressionensemble.lr_scalar"
                        )
            if self.config.get("model") == "averageensemble":
                # when "averageensemble.updata_lr_scalar" is set to TRUE, update the learning rates of the base learner all together with a scalar
                if self.model.config.get("averageensemble.updata_lr_scalar"):
                    lr_ori = self.optimizer_base_01.param_groups[0]["lr"]
                    self.optimizer_base_01.lr = lr_ori * self.config.get(
                        "averageensemble.lr_scalar"
                    )
                    lr_ori = self.optimizer_base_02.param_groups[0]["lr"]
                    self.optimizer_base_02.lr = lr_ori * self.config.get(
                        "averageensemble.lr_scalar"
        ...
        # to check here the status of the "requires_grad" of parameters.
        # this is because, in the case of "resume", the optimizer(s) loaded
        # will automatically set the "reguires_grad" to TRUE, even the related parameters are set FALSE.
        # so, do one more time checking before the training process starts, so to make it either correctly "not freezed" or "freezed"
        if (
            self.config.get("model") == "averageensemble"
            and self.config.get("freeze_param") == ""
        ):
            self.config.set("freeze_param", "meta")
        if (
            self.config.get("model") == "averageensemble"
            and self.config.get("freeze_param") == "base"
        ):
            self.config.set("freeze_param", "all")
        if self.config.get("model") in [
            "averageensemble",
            "logisticregressionensemble",
        ]:
            if self.config.get("freeze_param") in ["all", "base"]:
                self.model.model_base_01._freeze_param()
                self.model.model_base_02._freeze_param()
                if self.model.num_base > 2:
                    self.model.model_base_03._freeze_param()
                if self.model.num_base > 3:
                    self.model.model_base_04._freeze_param()
                self.model.aggregate_coef.requires_grad = False
            elif self.config.get("freeze_param") == "meta":
                self.model.model_base_01._unfreeze_param()
                self.model.model_base_02._unfreeze_param()
                if self.model.num_base > 2:
                    self.model.model_base_03._unfreeze_param()
                if self.model.num_base > 3:
                    self.model.model_base_04._unfreeze_param()
                self.model.aggregate_coef.requires_grad = False
            else:
                self.model.model_base_01._unfreeze_param()
                self.model.model_base_02._unfreeze_param()
                ...
                if self.model.num_base > 3:
                    self.model.model_base_04._unfreeze_param()
                self.model.aggregate_coef.requires_grad = True
        ...
        while True:
            ...
            if (
                ...
                if self.config.get("model") in [
                    ...
                    if self.config.get("freeze_param") == "all":
                        pass
                    elif (
                        self.config.get("freeze_param") == "base"
                        and self.config.get("model") == "logisticregressionensemble"
                    ):
                        pass
                    else:
                        if (
            ...
            if (
                ...
                if self.config.get("model") in [
                    ...
                    # for joint training
                    self.kge_lr_scheduler_base_01.step(trace_entry[metric_name])
                    self.kge_lr_scheduler_base_02.step(trace_entry[metric_name])
                    if self.model.num_base > 2:
                        self.kge_lr_scheduler_base_03.step(trace_entry[metric_name])
                    if self.model.num_base > 3:
                        self.kge_lr_scheduler_base_04.step(trace_entry[metric_name])
                    if self.config.get("model") == "logisticregressionensemble":
                        self.kge_lr_scheduler_meta.step(trace_entry[metric_name])
                else:
                ...
                if self.config.get("model") in [
                    ...
                    # for joint training
                    self.kge_lr_scheduler_base_01.step()
                    self.kge_lr_scheduler_base_02.step()
                    if self.model.num_base > 2:
                        self.kge_lr_scheduler_base_03.step()
                    if self.model.num_base > 3:
                        self.kge_lr_scheduler_base_04.step()
                    if self.config.get("model") == "logisticregressionensemble":
                        self.kge_lr_scheduler_meta.step()
                else:
        ...
        if self.config.get("model") in [
            ...
            print(self.model.aggregate_coef)
            ...
            if self.config.get("model") == "logisticregressionensemble":
                if self.model.num_base > 3:
                    train_checkpoint = {
                        ...
                        "optimizer_state_dict_meta": self.optimizer_meta.state_dict(),
                        "optimizer_state_dict_base_01": self.optimizer_base_01.state_dict(),
                        "optimizer_state_dict_base_02": self.optimizer_base_02.state_dict(),
                        "optimizer_state_dict_base_03": self.optimizer_base_03.state_dict(),
                        "optimizer_state_dict_base_04": self.optimizer_base_04.state_dict(),
                        "lr_scheduler_state_dict_base_01": self.kge_lr_scheduler_base_01.state_dict(),
                        "lr_scheduler_state_dict_base_02": self.kge_lr_scheduler_base_02.state_dict(),
                        "lr_scheduler_state_dict_base_03": self.kge_lr_scheduler_base_03.state_dict(),
                        "lr_scheduler_state_dict_base_04": self.kge_lr_scheduler_base_04.state_dict(),
                        "job_id": self.job_id,
                    }
                elif self.model.num_base > 2:
                    train_checkpoint = {
                        "type": "train",
                        "epoch": self.epoch,
                        "valid_trace": self.valid_trace,
    ...
    def _load(self, checkpoint: Dict) -> str:
        ...
        if self.config.get("model") in [
            ...
            if self.config.get("model") == "logisticregressionensemble":
                self.optimizer_meta.load_state_dict(
                    checkpoint["optimizer_state_dict_meta"]
                )
            self.optimizer_base_01.load_state_dict(
                checkpoint["optimizer_state_dict_base_01"]
            )
            self.optimizer_base_02.load_state_dict(
                checkpoint["optimizer_state_dict_base_02"]
            )
            if self.model.num_base > 2:
                self.optimizer_base_03.load_state_dict(
                    checkpoint["optimizer_state_dict_base_03"]
                )
            if self.model.num_base > 3:
                self.optimizer_base_04.load_state_dict(
                    checkpoint["optimizer_state_dict_base_04"]
                )
        else:
        ...
        if "lr_scheduler_state_dict_meta" in checkpoint:
            #     # new format
            self.kge_lr_scheduler_meta.load_state_dict(
                checkpoint["lr_scheduler_state_dict_meta"]
            )
        if "lr_scheduler_state_dict_base_01" in checkpoint:
            # new format
            self.kge_lr_scheduler_base_01.load_state_dict(
                checkpoint["lr_scheduler_state_dict_base_01"]
            )
        if "lr_scheduler_state_dict_base_02" in checkpoint:
            # new format
            self.kge_lr_scheduler_base_02.load_state_dict(
                checkpoint["lr_scheduler_state_dict_base_02"]
            )
        if "lr_scheduler_state_dict_base_03" in checkpoint:
            # new format
            self.kge_lr_scheduler_base_03.load_state_dict(
                checkpoint["lr_scheduler_state_dict_base_03"]
            )
        if "lr_scheduler_state_dict_base_04" in checkpoint:
            # new format
            self.kge_lr_scheduler_base_04.load_state_dict(
                checkpoint["lr_scheduler_state_dict_base_04"]
            )
    ...
    def run_epoch(self) -> Dict[str, Any]:
        ...
        for batch_index, batch in enumerate(self.loader):
            ...
            if self.config.get("model") in [
                ...
                self.optimizer_base_01.zero_grad()
                self.optimizer_base_02.zero_grad()
                if self.model.num_base > 2:
                    self.optimizer_base_03.zero_grad()
                if self.model.num_base > 3:
                    self.optimizer_base_04.zero_grad()
            else:
            ...
            if self.config.get("model") in [
                "averageensemble",
                "logisticregressionensemble",
            ] and self.config.get("freeze_param") in ["all", "base"]:
                pass
            ...
            if self.config.get("model") in [
                ...
                self.optimizer_base_01.step()
                self.optimizer_base_02.step()
                if self.model.num_base > 2:
                    self.optimizer_base_03.step()
                if self.model.num_base > 3:
                    self.optimizer_base_04.step()
            else:
                self.optimizer.step()
            ...
            if (
                ...
                in ["averageensemble", "logisticregressionensemble"]
                and self.config.get("freeze_param") == "all"
            ):
                pass
            elif (
                self.config.get("model")
            ...
            if self.config.get("model") == "logisticregressionensemble":
                if self.model.num_base > 3:
                    trace_entry = dict(
                        ...
                        lr_meta=[
                            group["lr"] for group in self.optimizer_meta.param_groups
                        ],
                        lr_base_01=[
                            group["lr"] for group in self.optimizer_base_01.param_groups
                        ],
                        lr_base_02=[
                            group["lr"] for group in self.optimizer_base_02.param_groups
                        ],
                        lr_base_03=[
                            group["lr"] for group in self.optimizer_base_03.param_groups
                        ],
                        lr_base_04=[
                            group["lr"] for group in self.optimizer_base_04.param_groups
                        ],
...
class TrainingJobMetaLearner(TrainingJob):
    """Samples SPO pairs and queries sp_ and _po, treating all other entities as negative."""

    def __init__(self, config, dataset, parent_job=None, model=None):
        super().__init__(config, dataset, parent_job, model=model)
        config.log("Initializing spo training job...")
        self.type_str = "MetaLearner"

        if self.__class__ == TrainingJobMetaLearner:
            for f in Job.job_created_hooks:
                f(self)
    ...
    def _prepare(self):
        ...
        super()._prepare()
        self.num_examples = self.dataset.split(self.train_split).size(0)
        self.triples = self.dataset.split(self.config.get("train.split"))
        self.loader = torch.utils.data.DataLoader(
            self.triples,
            shuffle=False,
            batch_size=self.batch_size,
    ...
    def _process_batch(self, batch_index, batch) -> TrainingJob._ProcessBatchResult:
        ...
        scores = self.model.score_sp_po_meta(
            s, p, o,  # torch.arange(0, num_entities).to(self.device)
        )

        X = torch.transpose(scores, 0, 1).detach().cpu().numpy()

        y_range = int(len(scores[0]) / (num_entities * 2))

        y_train_sp = np.zeros((y_range, num_entities))
        y_train_po = np.zeros((y_range, num_entities))

        for i in range(y_range):
            y_train_sp[i, o[i]] = 1.0
            y_train_po[i, s[i]] = 1.0

        y_train = np.concatenate(np.hstack((y_train_sp, y_train_po,),))

        y = np.transpose(y_train)

        if self.model.meta_prepared:
            self.model.regression.fit(
                X=X,
                y=y,
                coef_init=self.aggregate_coef[: self.num_base]
                .view(-1)
                .detach()
                .cpu()
                .numpy(),
                intercept_init=self.aggregate_coef[self.num_base]
                .detach()
                .cpu()
                .numpy(),
            )
        else:
            self.model.regression.partial_fit(X, y, [0, 1])

        coef = self.model.regression.coef_[0].tolist()
        coef.extend(self.model.regression.intercept_.tolist())
        import numpy

        self.model.aggregate_coef = torch.nn.Parameter(
            torch.from_numpy(numpy.array(coef)).float().view(-1, 1).to(self.device)
        )
