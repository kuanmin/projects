...
class EntityRankingJob(EvaluationJob):
    ...
    def _run(self) -> dict:
        ...
        for batch_number, batch_coords in enumerate(self.loader):
            ...
            # score table
            # consider if the scores are from different models
            ...
            for chunk_number in range(math.ceil(num_entities / chunk_size)):
                ...
                # while using multiple models, don't replace the socres in scores_sp / scores_po with the scores in o_true_scores / s_true_scores
                # since the resource of o_true_scores / s_true_scores is from scores_spo, which is not be able to perform minmax on.
                # on the other hand, the resource of scores_sp / scores_po is from scores_sp_po, which has been mini-maxed.
                if self.config.get("model") in [
                    "averageensemble",
                    "logisticregressionensemble",
                ]:
                    o_true_scores[o_in_chunk_mask] = scores_sp[
                        o_in_chunk_mask, o_in_chunk
                    ]
                    s_true_scores[s_in_chunk_mask] = scores_po[
                        s_in_chunk_mask, s_in_chunk
                    ]
                else:
                    scores_sp[o_in_chunk_mask, o_in_chunk] = o_true_scores[
                        o_in_chunk_mask
                    ]
                    scores_po[s_in_chunk_mask, s_in_chunk] = s_true_scores[
                        s_in_chunk_mask
                    ]
