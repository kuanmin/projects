...
class KLDivWithSoftmaxKgeLoss(KgeLoss):
    ...
    def __call__(self, scores, labels, **kwargs):
        if labels.dim() == 1:
            ...
            # print("this is the scores in KKK.......\n", scores)
            # print("this is the scores in KKK..2.....\n", labels)
            # print(
            #     "this is the self._klloss(sim=1))........",
            #     self._klloss(scores, F.normalize(labels.float(), p=1, dim=1)),
            # )
            # if self.config.get("model") == "logisticregressionensemble":
            #     return self._klloss(scores, F.normalize(labels.float(), p=1, dim=1))
            # else:
            return self._klloss(
